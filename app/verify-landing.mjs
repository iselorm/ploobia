...
/* ------------------------------------------------------------------------ */
{
  ...
  check('the checklist ticks Take the plot', (await page.locator('[data-testid=checklist] li[data-done=true]').count()) === 1)
  ...
  check('the checklist ticks Say how many cans', (await page.locator('[data-testid=checklist] li[data-done=true]').count()) >= 3)
  ...
  check('Explorer sees words: accuracy spot on, economy the fewest', (await page.getByTestId('record-accuracy').textContent()).includes('spot on') && (await page.getByTestId('record-economy').textContent()).includes('fewest'))
  ...
  check("'it was thirsty' is disproved by the world, in Ploob's line", await page.getByTestId('plot-why-line').textContent().then((t) => /Nobody gave it any/.test(t)))
...
for (const [name, width, height] of [
  ...
  check(`${name}: the plate collapses to one line`, /Day 1 of 14 · firm 0\.\d\d · cans 0 of 2 said/.test(line), line)
