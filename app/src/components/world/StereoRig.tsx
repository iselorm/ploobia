 ...
 * where the head points. Position still comes from the cabinet's authored
 * viewpoint flights; this rig owns orientation only, and only while stereo is
 * on. Drag is the fallback when there is no gyroscope.
 *
 ...
 *
 * Platform primitive. It takes the current viewpoint as data rather than
 * importing one cabinet's viewpoint table, so every cabinet with authored
 * shots can offer Cardboard.
 */

export interface StereoViewpoint {
  label: string
...
interface Props {
  /** The shot the camera is flying to, or null for "wherever it is". */
  view: StereoViewpoint | null
  /** Bumped whenever a new shot is requested; recalibrates "forward". */
  viewSeq: number
}

export default function StereoRig({ view, viewSeq }: Props) {
  ...
  const device = useRef<{ alpha: number; beta: number; gamma: number; has: boolean; lastAt: number }>({
    alpha: 0,
    beta: 0,
    gamma: 0,
    has: false,
  ...
  // Children of the camera (reticle, hint) only render if the camera is in the
  // scene graph.
  useEffect(() => {
    if (!stereo.on) return
    scene.add(camera)
  ...
  useFrame(() => {
    ...
    q.target.set(
      view ? view.target[0] : 0,
      view ? view.target[1] : 1.6,
      view ? view.target[2] : 0,
    )
    if (calib.current.viewSeq !== viewSeq) {
      calib.current.viewSeq = viewSeq
      ...
      const dest = view ? view.position : [camera.position.x, camera.position.y, camera.position.z]
      q.scratch.set(dest[0], dest[1], dest[2])
      q.fwd.subVectors(q.target, q.scratch)
  ...
  const hint = useMemo(
    () => glyphTexture(view ? `${view.label} · tap for next` : 'tap for next', '#FBF5EA'),
    [view],
  )
...
/** Standard device-orientation → world quaternion (as DeviceOrientationControls did). */
function computeDeviceQuaternion(
  d: { alpha: number; beta: number; gamma: number },
  q: {
    device: THREE.Quaternion
    euler: THREE.Euler
    q0: THREE.Quaternion
    q1: THREE.Quaternion
    zee: THREE.Vector3
