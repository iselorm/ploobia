...
export function glyphTexture(text: string, color: string, style: GlyphStyle = {}): { texture: THREE.CanvasTexture; aspect: number } {
  const strokeWidth = style.strokeWidth ?? 14
  const strokeColor = style.strokeColor ?? 'rgba(251, 245, 234, 0.92)'
  const key = `${text}|${color}|${strokeWidth}|${strokeColor}`
  ...
  if (ctx) {
    ...
    ctx.font = `700 ${text.length > 8 ? 62 : 76}px Nunito, ui-rounded, system-ui, sans-serif`
...
const GlyphInstances = forwardRef<THREE.InstancedMesh, Props>(function GlyphInstances(
  { text, color, count, size = 0.26, style },
  ref,
) {
  const { texture, aspect } = useMemo(() => glyphTexture(text, color, style), [text, color, style])

  return (
    <instancedMesh ref={ref} args={[undefined, undefined, count]} frustumCulled={false} renderOrder={2}>
