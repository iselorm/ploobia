...
## 8b. The Archipelago (the world edition — on the trunk since 2026-09-23)

The world is **the same app with `VITE_WORLD=1`**, not a second product. `world` merged into `main` at the `bible-v0.4` tag; from here rounds are short branches off `main` (`s0-plot`, `s1-handoff`, …) merged when their gate passes. The governing design is **Archipelago Bible v0.4 — Keep, Arm, Defend** (a Claude Doc; companion: the Storyline and Development Plan v2). Read it before any world work; the World Bible v0.2 still holds the locked decisions it does not reopen.

- **Two builds from one tree.** `npx vite build` = the arcade, offline, **must contain no "rapier"** (grep the bundle). `VITE_WORLD=1 npx vite build --outDir dist-world` = the world, online-first (Rapier, models under `public/models/world/`, the judge at `functions/api/why.js`). Cloudflare Pages Preview builds the world; Production does not until Selorm says so.
- **Suites.** `verify-world.mjs` against `dist-world` on :8766 (phone sections at 844×390 and 740×360, one portrait, hit-tests per tier); every arcade suite unchanged against `dist` on :8765. Run them sequentially, never in parallel.
- **Rules that carry from the courtyard:** the client is not the coach (Sefu's lines are testimony, never instruction, never an unmeasured number); predicates typed, never `eval`; the scene renders state and the store is written at ~8 Hz, never per frame; a door into a cabinet is `location.hash = '#/<cabinet>?from=world&door=N'` and the cabinet reports back through the store; the explorer's clips are retargeted with `scripts/retarget-clip.mjs` (chest ahead of the waist, never waist-led); `npx prettier --write` is never run (no config in the repo); `tsc --noEmit -p tsconfig.app.json` is the type check (`tsc -p .` checks nothing).
- **The settlement layer (v0.4):** the child starts with a plot at the Landing; teaching an NPC is the hand-in (judge verdict → competence, confirmed by a transfer case); supplies enable *named* things, never multipliers; the Hollow Fleet lands where the Lens shows red; three clocks (real · trial · campaign); the Codex is diegetic Field Guide pages that never replace a measurement; titles are per island. Prototype = S0 plot → S1 handoff → S2 courtyard with a reason → S3 warning and watch → S4 Hold the Jetty → S5 first bronze.
