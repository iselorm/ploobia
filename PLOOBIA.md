...
## 5b. The Book (the Enhanced Textbook's page engine — every cabinet, eventually)

**Decided 2026-09-11: the chapter is the other front door.** A chapter is the IGCSE topic in the syllabus's own order; a section is three or four narrated pages where every academic term is a verb in the cabinet; the section's **practical is one of the campaign doors, run exactly as §5 runs it**; a hand-in that *hits* stamps the syllabus statements it evidenced. Vault: `Enhanced Textbook.md`, `The Library.md`; first build `Build Log — The Sugar Line IV · The Book.md`.

1. **A page you can only read is a to-do list.** A term is braced only if it resolves to a registered verb (`lib/verbs.ts`): `{term:object/verb}` · `{term:object/verb|gloss}` · `{{ext}}…{{/ext}}` (Analyst only). Every verb carries a **probe**; the suite taps every word and asserts the sim moved. A word with nothing to show is a build error.
2. **One page source, three band layers** (`simple / formal / technical`, picked by `BAND_CAPS.vocab`), **six language keys** (`en tw ha fr sw ar`; the academic term stays English; `ar` is RTL — logical CSS properties only; Western digits everywhere by decision). Verbs are never translated.
3. **The practical is the door, unchanged.** `practical: { stage, preset per band, stamps per band }` — two sections can share a door and ask for different levels. The book adds nothing to the round and takes nothing from it; **score never touches the trial**.
4. **A stamp is evidence, not XP**, and it claims only what was evidenced: a *hit* at the section's own level for that band, or a check-page answer **committed before the model answer unfolds** (Explorer picks · Scientist orders · Analyst writes and self-marks against the points). A miss stamps nothing and says so. Where a section does not cover a statement, the ledger says *not here*. Ledger key `ploobia.stamps.v1`; statements in `lib/curriculum.ts`, verbatim from the syllabus, numbered in its list order.
5. **A check page only where the practical cannot stamp**, and only at bands with such a statement (`pagesFor`).
6. **Layout is the cabinet's own tiers** (§6): the page card **replaces the parts column** on desktop/tablet; a *Page* edge tab and left sheet on the phone; the turn card in portrait. Tappable words get their room from the line (line-height and padding on touch), never the centred hit-area extension.
7. **Where the cabinet has no stage for a part, the page carries its own figure** (`SectionFigure`, hotspot map = the contract for a painted card) — the syllabus's "identify in diagrams and images".
8. **Read to me** walks the words with Web Speech (`narrator.walk`), firing verbs as it reaches them; nothing audible is load-bearing; recorded voice packs later.
9. **Ways in:** *Read the chapter* on the welcome, the **Book** chip beside Challenge, `?book=<section>&page=<n>`. The contents card is the cabinet's until the Library takes it into the hall (phase 2).
10. Suite: `verify-book.mjs`; new cabinets register their verbs in `lib/<cabinet>verbs.ts` and ship their sections in `src/books/<subject>/`.
